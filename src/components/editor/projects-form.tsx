    ...
    <div className="space-y-4 py-2">
      {projects.map((proj, i) => (
        <div key={i} className="border rounded p-3 pr-8 space-y-2 relative min-w-0">
          ...
          <input className="border rounded px-2 py-1 text-sm w-full min-w-0" placeholder="项目名称" value={proj.name} onChange={(e) => update(i, "name", e.target.value)} />
          <textarea className="border rounded px-2 py-1 text-sm w-full min-w-0 box-border" placeholder="项目描述" rows={2} value={proj.description} onChange={(e) => update(i, "description", e.target.value)} />
          <input className="border rounded px-2 py-1 text-sm w-full min-w-0" placeholder="链接 (可选)" value={proj.url || ""} onChange={(e) => update(i, "url", e.target.value)} />
        </div>
      ))}
